...
class IntentPredictionService {
  ...
  Future<UserIntent> predictUserIntent({
    ...
    try {
      ...
      final recentIntents = await _memoryService.getRecentIntents(_currentUserId!, limit: 20);
      
      // Analyze conversation context
      final contextAnalysis = _analyzeConversationContext(conversationHistory, currentContext);
      ...
      final predictedIntent = _selectBestIntent(intentProbabilities, messageContent);
      
      // Store the predicted intent for learning
      await _memoryService.storeUserIntent(predictedIntent, _currentUserId!, sessionId);
      
      if (kDebugMode) {
        print('Predicted intent: ${predictedIntent.type.displayName} (${(predictedIntent.confidence * 100).toStringAsFixed(1)}%)');
      }
  ...
  Map<IntentType, double> _analyzeMessageContent(String content) {
    ...
    if (content.contains('?')) {
      scores[IntentType.questionAnswering] = (scores[IntentType.questionAnswering] ?? 0.0) + 0.5;
    }
    
    if (content.contains('!') || content.toUpperCase() == content) {
      scores[IntentType.troubleshooting] = (scores[IntentType.troubleshooting] ?? 0.0) + 0.3;
    }
    
    if (content.length > 200) {
      scores[IntentType.problemSolving] = (scores[IntentType.problemSolving] ?? 0.0) + 0.2;
    }
    
    // Check for continuation words
    final continuationWords = ['also', 'and', 'furthermore', 'moreover', 'additionally'];
    for (final word in continuationWords) {
      if (lowercaseContent.contains(word)) {
        scores[IntentType.continuation] = (scores[IntentType.continuation] ?? 0.0) + 0.3;
        break;
      }
    }
  ...
  Map<IntentType, double> _analyzeConversationContext(
    ...
    final hasProblemKeywords = recentMessages.any((msg) => 
    ...
    final hasLearningKeywords = recentMessages.any((msg) => 
    ...
    if (currentContext != null) {
      ...
      for (final topic in currentContext.activeTopics) {
        if (topic.toLowerCase().contains('image') || topic.toLowerCase().contains('photo')) {
          scores[IntentType.imageAnalysis] = (scores[IntentType.imageAnalysis] ?? 0.0) + 0.2;
        }
        if (topic.toLowerCase().contains('problem') || topic.toLowerCase().contains('issue')) {
          scores[IntentType.problemSolving] = (scores[IntentType.problemSolving] ?? 0.0) + 0.2;
        }
      }
  ...
  Future<Map<IntentType, double>> _getBehaviorPatterns() async {
    ...
    try {
      ...
      for (final topic in topicMemory) {
        ...
        if (topicName.toLowerCase().contains('image') || topicName.toLowerCase().contains('photo')) {
          scores[IntentType.imageAnalysis] = (scores[IntentType.imageAnalysis] ?? 0.0) + (interestScore * 0.3);
        }
        if (topicName.toLowerCase().contains('problem') || topicName.toLowerCase().contains('debug')) {
          scores[IntentType.problemSolving] = (scores[IntentType.problemSolving] ?? 0.0) + (interestScore * 0.3);
        }
        if (topicName.toLowerCase().contains('learn') || topicName.toLowerCase().contains('tutorial')) {
          scores[IntentType.learning] = (scores[IntentType.learning] ?? 0.0) + (interestScore * 0.3);
        }
      }
  ...
  Map<IntentType, double> _calculateIntentProbabilities({
    ...
    final Map<IntentType, double> timeContextScores = _calculateTimeContextScores(currentContext);
  ...
  Map<IntentType, double> _calculateTimeContextScores(ConversationContext? currentContext) {
    final Map<IntentType, double> scores = {};
  ...
  UserIntent _selectBestIntent(Map<IntentType, double> probabilities, String messageContent) {
  ...
  List<String> _extractParameters(String content, IntentType intentType) {
    ...
    switch (intentType) {
      case IntentType.imageAnalysis:
        if (content.toLowerCase().contains('analyze')) parameters.add('analysis_requested');
        if (content.toLowerCase().contains('describe')) parameters.add('description_requested');
        if (content.toLowerCase().contains('identify')) parameters.add('identification_requested');
        break;
      ...
      case IntentType.problemSolving:
        if (content.toLowerCase().contains('error')) parameters.add('error_mentioned');
        if (content.toLowerCase().contains('fix')) parameters.add('fix_requested');
        if (content.toLowerCase().contains('solve')) parameters.add('solution_requested');
        break;
        
      case IntentType.learning:
        if (content.toLowerCase().contains('learn')) parameters.add('learning_requested');
        if (content.toLowerCase().contains('explain')) parameters.add('explanation_requested');
        if (content.toLowerCase().contains('understand')) parameters.add('understanding_requested');
        break;
        
      case IntentType.comparison:
        if (content.toLowerCase().contains('vs') || content.toLowerCase().contains('versus')) {
          parameters.add('comparison_requested');
        }
        if (content.toLowerCase().contains('better')) parameters.add('preference_requested');
        break;
  ...
  Future<List<Map<String, dynamic>>> predictNextActions({
    ...
    try {
      final recentIntents = await _memoryService.getRecentIntents(_currentUserId!, limit: 10);
      ...
      if (conversationHistory.isNotEmpty) {
        ...
        if (lastMessage.type == MessageType.ai) {
          predictions.add({
            ...
            'probability': 0.7,
            'suggested_response': 'Would you like me to explain anything further?',
          });
        }
        
        // Predict image upload
        if (lastMessage.content.toLowerCase().contains('image') && lastMessage.imagePaths.isEmpty) {
          predictions.add({
            'action': 'image_upload',
            'description': 'User might upload an image for analysis',
            'probability': 0.6,
            'suggested_response': 'Feel free to upload an image if you\'d like me to analyze it.',
          });
        }
        
        // Predict continuation
        if (recentIntents.isNotEmpty && recentIntents.first.type == IntentType.learning) {
          predictions.add({
            'action': 'continue_learning',
            'description': 'User might want to learn more about the topic',
            'probability': 0.5,
            'suggested_response': 'Would you like to explore this topic further?',
          });
        }
      }
      ...
      if (currentContext != null) {
        ...
        if (timeOfDay == 'evening' || timeOfDay == 'night') {
          predictions.add({
            ...
            'probability': 0.4,
            'suggested_response': 'Is there anything else I can help you with today?',
          });
        }
      }
  ...
  Future<double> getIntentPredictionConfidence(String sessionId) async {
    ...
    try {
      final recentIntents = await _memoryService.getRecentIntents(_currentUserId!, limit: 5);
      if (recentIntents.isEmpty) return 0.5;
      
      // Calculate average confidence of recent predictions
      final totalConfidence = recentIntents.fold(0.0, (sum, intent) => sum + intent.confidence);
  ...
  Future<void> updateIntentPredictionAccuracy({
    ...
    try {
      ...
      await _memoryService.storeContextMemory(
        ...
        importanceScore: wasAccurate ? 0.8 : 0.9, // Higher importance for incorrect predictions
      );
